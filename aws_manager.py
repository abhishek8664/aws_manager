...
class AWSManager:
            ...
            if response["Reservations"]:
                for reservation in response["Reservations"]:
                    for instance in reservation["Instances"]:
                        ...
                        self.logger.info(
                            f"Started EC2 instance {instance_id} ({instance_name})"
                        )
            else:
            ...
            if response["Reservations"]:
                for reservation in response["Reservations"]:
                    for instance in reservation["Instances"]:
                        ...
                        self.logger.info(
                            f"Stopped EC2 instance {instance_id} ({instance_name})"
                        )
            else:
    ...
    def ec2_asg_desired_capacity(
        self, desired_count: int, min_count: int, max_count: int
    ) -> None:
        try:
    ...
    def check_asg_status(self) -> Table:
        ...
        for asg in asg_response["AutoScalingGroups"]:
            ...
            table.add_row(
                asg_name,
                str(current_min_capacity),
                str(current_max_capacity),
                str(current_desired_capacity),
            )
        return table
    ...
    if choice == "0":
        ...
        with Progress(
            ...
            task = progress.add_task(
                "[bold cyan]Checking Resource(EC2, RDS, ASG) status", total=None
            )
            awsman.check_status()
        ...
        with Progress(
            ...
            task = progress.add_task(
                "[bold cyan]Scaling Auto Scaling Group up", total=None
            )
        ...
        with Progress(
            ...
            task = progress.add_task(
                "[bold cyan]Scaling Auto Scaling Group down", total=None
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="AWS Manager", formatter_class=RichHelpFormatter
    )
    ...
    if args.list:
        # awsman.list_resources(args.list)
        if args.list == "all":
            awsman.check_status()
        elif args.list == "ec2":
            print(awsman.check_ec2_status())
        elif args.list == "rds":
            print(awsman.check_rds_status())
        elif args.list == "asg":
            print(awsman.check_asg_status())
